# 🚩 CTF Writeups

**Author:** Chad Hackerman
**Competition Experience:** 50+ CTF events
**Notable Achievements:** Top 10% in multiple national competitions

Collection of Capture the Flag competition solutions demonstrating penetration testing methodologies and problem-solving approaches.

---

## 🏆 Competitive Results

| Competition | Result |
|-------------|--------|
| DEF CON CTF Qualifier 2023 | Top 15% globally |
| SANS NetWars Tournament | 2nd place regional |
| PicoCTF 2023 | Top 5% of 10,000+ participants |

---

## 📂 Writeups

### Web Exploitation
- [SQL Injection — Blind Time-Based](./writeups/web-sql-injection.md)

### Cryptography
- [RSA Weak Keys — Common Modulus Attack](./writeups/crypto-rsa-weak-keys.md)

### Binary Exploitation
- [Buffer Overflow — ASLR Bypass + ROP Chain](./writeups/pwn-buffer-overflow.md)

### Reverse Engineering
- [Malware Analysis — C2 Protocol Recovery](./writeups/re-malware-analysis.md)

---

## 🛠️ Tools & Skills Demonstrated

| Category | Tools |
|----------|-------|
| Web Exploitation | Burp Suite, SQLMap, custom Python scripts |
| Cryptography | Python, gmpy2 |
| Binary Exploitation | GDB, pwntools, ROPgadget |
| Reverse Engineering | IDA Pro, Ghidra, x64dbg |

---

*Part of [Chad Hackerman's Portfolio](https://github.com/chad-hackerman/chad-hackerman-portfolio)*
